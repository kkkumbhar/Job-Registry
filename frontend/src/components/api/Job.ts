...
import type { PaginationParams, APIResponse, QueryOptions } from "../../types/tabletype";
import {
    CACHE_TIME_SHORT,
...
import type { Attachment, DataProtectionJob, LogEntry } from "../../types/jobs";
...
const mapBackendStatus = (status?: string): "running" | "successful" | "failed" | "cancelled" => {
    if (!status) return "running";

    const statusUpper = status.toUpperCase();

    // Handle enum format (eJOB_*)
    if (statusUpper === "EJOB_SUCCEEDED" || statusUpper.includes("SUCCESS") || statusUpper === "COMPLETED") {
        return "successful";
    }
    if (statusUpper === "EJOB_FAILED" || statusUpper.includes("FAIL") || statusUpper.includes("ERROR")) {
        return "failed";
    }
    if (
...
const monitoringApi = {
    useGetJobsQuery: (params?: QueryParams & PaginationParams, options?: QueryOptions) => {
                    ...
                    items: (backendData.data || []).map((job: any) => ({
                        ...
                        user:
                            typeof job.metadata?.user === "string"
                                ? job.metadata.user
                                : job.metadata?.user?.username || job.metadata?.user?.name || null,
                        userTags: job.metadata?.tags?.map((tag: any) => `${tag.key}:${tag.value}`) || [],
                        policyName: null,
    ...
    useGetJobQuery: (jobId?: string, options?: QueryOptions) => {
        return useQuery<DataProtectionJob>({
            queryKey: ["job-cache", "details", jobId],
            queryFn: async () => {
                if (!jobId) throw new Error("Job ID is required");

                // For now, get all jobs and find the one we need
                // You may want to add a specific endpoint for single job retrieval
                const response = await dataProtectionClient.get(`/jobs`);
                const job = response.data.data?.find((j: any) => j.external_id === jobId);
            ...
            staleTime: ITEM_CACHE_TIME,
            refetchInterval: getRefreshInterval(options?.refresh),
        });
    },

    useGetJobLogsQuery: (jobId: string, params?: PaginationParams, options?: QueryOptions) => {
        const queryParams: Record<string, unknown> = { order: "sequenceNumber DESC", ...params };
        return useQuery<APIResponse<LogEntry>>({
            queryKey: ["job-log-cache", jobId, params],
            queryFn: async () => {
                return (await dataProtectionClient.get(`/jobs/${jobId}/logs/`, { params: queryParams })).data;
            },
    ...
    useGetLogAttachmentsQuery: (logId: string, options?: QueryOptions & { download?: boolean }) => {
        return useQuery<APIResponse<Attachment> | Blob>({
            queryKey: ["job-log-attachment-cache", logId],
            queryFn: async () => {
                const response = await dataProtectionClient.get(`/logs/${logId}/attachments/`, {
                    responseType: options?.download ? "blob" : undefined,
                });
                return options?.download ? (response.data as Blob) : (response.data as APIResponse<Attachment>);
            },
    ...
    useDownloadJobLogsMutation: (jobId: string) => {
        return useMutation({
            mutationFn: async () => {
                const response = await dataProtectionClient.get(`jobs/${jobId}/export-logs/`, {
                    responseType: "blob",
                });
...
export async function fetchJobById(id: string): Promise<DataProtectionJob> {
    const response = await dataProtectionClient.get(`/jobs`);
    const job = response.data.data?.find((j: any) => j.external_id === id);
...
export const useGetJobStatsQuery = (params?: { sources?: string[]; name?: string }, options?: QueryOptions) => {
    return useQuery<{ total: number; successful: number; failed: number; running: number }>({
